...
export class HeadPoseDetector {
  ...
  constructor(opts: HeadPoseDetectorOptions = {}) {
    ...
    this.minTrackingConfidence = opts.minTrackingConfidence ?? DEFAULT_CONFIDENCE;
    ...
    this.faceLandmarker = await silenceMediapipeInfo(async () => {
      ...
      if (this.preferGpu) {
        try {
          ...
          console.warn("[HeadPoseDetector] GPU delegate failed, falling back to CPU.", e);
        }
      }
  ...
  detectFromVideo(video: HTMLVideoElement): HeadPoseResult {
    ...
    this.lastResult = { yaw: outYaw, pitch: outPitch, roll: outRoll, faceDetected: true, landmarks };
    return this.lastResult;
  }
...
function matrixToEuler(matrix: { data: Float32Array | number[] }): { yaw: number; pitch: number; roll: number } {
  const m = matrix.data;
