...
function eyeAspectRatio(landmarks: Point2D[], eyeIndices: number[]): number | null {
...
export class BlinkDetector {
  ...
  private lastResult: BlinkResult = { blinkDetected: false, ear: null, leftEar: null, rightEar: null, leftBlinking: false, rightBlinking: false, faceDetected: false, calibrated: false };
  private pendingBlink = false;

  constructor(opts: BlinkDetectorOptions = {}) {
    this.opts = {
      faceLandmarkerModelUrl: opts.faceLandmarkerModelUrl ?? DEFAULT_FACE_LANDMARKER_MODEL,
  ...
  async load(): Promise<void> {
    ...
    if (this.opts.preferGpu) {
      try {
        this.faceLandmarker = await FL.createFromOptions(vision, {
          ...baseConf,
          baseOptions: { modelAssetPath: this.opts.faceLandmarkerModelUrl, delegate: "GPU" },
        });
        return;
      } catch {
        // fall through to CPU
    ...
    this.faceLandmarker = await FL.createFromOptions(vision, {
      ...baseConf,
      baseOptions: { modelAssetPath: this.opts.faceLandmarkerModelUrl, delegate: "CPU" },
    });
  }

  private updateEyeSmooth(eye: EyeState, raw: number, dt_ms: number, now: number): void {
    eye.prevSmoothed = eye.smoothed;
    const a = 1 - Math.exp(-dt_ms / EAR_SMOOTH_TAU_MS);
    eye.smoothed = eye.smoothed === null ? raw : eye.smoothed * (1 - a) + raw * a;

    if (eye.baseline === null) {
      if (eye.warmupStart === null) eye.warmupStart = now;
      eye.baselineSamples.push(raw);
      if (now - eye.warmupStart >= BASELINE_WARMUP_MS && eye.baselineSamples.length > 0) {
  ...
  private eyeBlinkEntry(eye: EyeState, dt_ms: number): boolean {
    ...
    if (eye.smoothed >= enter) { eye.lowMs = 0; return false; }
    const dropPerSec = eye.prevSmoothed !== null
      ? ((eye.prevSmoothed - eye.smoothed) / dt_ms) * 1000
      : 0;
    const fastEnough = dropPerSec >= EAR_MIN_DROP_PER_SEC || eye.lowMs > 0;
    if (!fastEnough) { eye.lowMs = 0; return false; }
    eye.lowMs += dt_ms;
  ...
  private bothEyesOpen(): boolean {
    ...
    return l.smoothed >= l.baseline * EAR_EXIT_RATIO && r.smoothed >= r.baseline * EAR_EXIT_RATIO;
  }
  ...
  detectFromVideo(video: HTMLVideoElement): BlinkResult {
    if (!this.faceLandmarker) return { ...this.lastResult, blinkDetected: false };
    ...
    if (now - this.lastInferenceTime < 20) return { ...this.lastResult, blinkDetected: false };
  ...
  private get leftBlinking(): boolean {
    ...
    return l.smoothed < l.baseline * EAR_ENTER_RATIO || l.blendLowMs >= BLENDSHAPE_MIN_LOW_MS;
  }

  private get rightBlinking(): boolean {
    const r = this.rightEye;
    if (r.baseline === null || r.smoothed === null) return false;
    return r.smoothed < r.baseline * EAR_ENTER_RATIO || r.blendLowMs >= BLENDSHAPE_MIN_LOW_MS;
  }
  ...
  private processResults(
    ...
    const dt_ms = this.lastInferenceTimestamp > 0
    ...
    if (!results.faceLandmarks?.length) {
      ...
      return {
        ...
        ear: null, leftEar: null, rightEar: null,
        leftBlinking: false, rightBlinking: false,
        faceDetected: false, calibrated: this.calibrated,
      };
    }
    ...
    if (!leftValid || !rightValid || leftEAR === null || rightEAR === null) {
      ...
      return {
        ...
        faceDetected: true, calibrated: this.calibrated,
      };
    }
  ...
  resetHistory(): void {
    ...
    this.pendingBlink = false;
    this.lastResult = { blinkDetected: false, ear: null, leftEar: null, rightEar: null, leftBlinking: false, rightBlinking: false, faceDetected: false, calibrated: false };
  }
