...
interface FaceLandmarksDetector {
  ...
  ): Promise<Array<{ keypoints: Array<{ x: number; y: number; z?: number; name?: string }> }>>;
}
...
export class BlinkDetectorNode {
  ...
  async detectFromImagePath(imagePath: string): Promise<BlinkResult> {
    ...
    const { data } = await sharp(imagePath).toFormat("raw").toBuffer({ resolveWithObject: true });
    const imageTensor = tf.tensor3d(new Uint8Array(data.buffer as ArrayBuffer), [height, width, 3]);

    const faces = await this.landmarksDetector.estimateFaces(imageTensor, { flipHorizontal: false });
    imageTensor.dispose();
  ...
  private processLandmarks(
    ...
    if (leftEAR === null || rightEAR === null) {
      this.lastResult = { ...this.lastResult, blinkDetected: false, faceDetected: true };
      return this.lastResult;
    }
  ...
  private updateEyeSmooth(eye: EyeState, raw: number): void {
    ...
    } else if (!this.blinking && eye.smoothed !== null && eye.smoothed >= eye.baseline) {
  ...
  private eyeBlinkEntry(eye: EyeState): boolean {
    ...
    if (eye.smoothed >= enter) { eye.lowFrames = 0; return false; }
    const drop = eye.prevSmoothed !== null ? eye.prevSmoothed - eye.smoothed : 0;
    const fastEnough = drop >= EAR_MIN_DROP || eye.lowFrames > 0;
    if (!fastEnough) { eye.lowFrames = 0; return false; }
    eye.lowFrames += 1;
