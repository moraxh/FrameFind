											...
											{!stream && (
												<div className="text-center p-6">
													...
													<p className="text-sm text-neutral-500 mt-1">
														Click start to test the detection
													</p>
												</div>
											)}
										</>
										...
										<div className="absolute bottom-4 left-1/2 -translate-x-1/2 bg-black/80 backdrop-blur-md border border-white/10 px-6 py-3 flex items-center gap-4 rounded-2xl shadow-xl transition-all duration-300">
											<div className="text-3xl">
												{!displayResult.faceDetected
													? "🫣"
													: displayResult.glasses
														? "👓"
														: "😊"}
											</div>
											<div className="text-left">
												<p
													className={`text-sm font-bold ${!displayResult.faceDetected ? "text-yellow-400" : displayResult.glasses ? "text-cyan-400" : "text-white"}`}
												>
													{!displayResult.faceDetected
														? "No Face Detected"
														: displayResult.glasses
												...
												{displayResult.faceDetected && (
													<p className="text-xs text-neutral-400">
														{(displayResult.probability * 100).toFixed(1)}%
														confidence
													</p>
												)}
												{inferenceTime !== null && (
						...
						<div className="space-y-6">
							<div>
								...
								<p className="text-neutral-400 leading-relaxed mb-4">
									No server calls. No tracking. Your camera and image data never
									leave your device.
								</p>
								<ul className="space-y-3">
									...
									<li className="flex gap-3">
										<span className="text-cyan-400 font-bold">→</span>
										<span className="text-neutral-300">
											<strong>Lightweight:</strong> 6.2MB model, works on any
											device
										</span>
									</li>
									<li className="flex gap-3">
										<span className="text-cyan-400 font-bold">→</span>
										<span className="text-neutral-300">
											<strong>Private:</strong> All processing happens locally
											in your browser
										</span>
									</li>
								</ul>
